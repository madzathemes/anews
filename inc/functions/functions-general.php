...
if ( ! function_exists( 'anews_paging_nav' ) ) :

function anews_paging_nav() {
	global $wp_query,  $anews_allowed_html_array;

	// Don't print empty markup if there's only one page.
	if ( $wp_query->max_num_pages < 2 )
		return;
		if ( false == get_theme_mod( 't_p_older_posts', false ) ) { $t_p_older_posts = wp_kses(__( '<span class="meta-nav">&larr;</span> Older posts', 'anews' ), $anews_allowed_html_array );  } else { $t_p_older_posts = get_theme_mod( 't_p_older_posts' ); $t_p_older_posts = wp_kses(( '<span class="meta-nav">&larr;</span> '.$t_p_older_posts), $anews_allowed_html_array ); }
		if ( false == get_theme_mod( 't_p_newer_posts', false ) ) { $t_p_newer_posts = wp_kses(__( 'Newer posts <span class="meta-nav">&rarr;</span>', 'anews' ), $anews_allowed_html_array );  } else { $t_p_newer_posts = get_theme_mod( 't_p_newer_posts' ); $t_p_newer_posts = wp_kses(( $t_p_newer_posts.' <span class="meta-nav">&rarr;</span>'), $anews_allowed_html_array ); }
...
if ( ! function_exists( 'anews_entry_meta' ) ) {
/**
 ...
 *
 */
	function anews_content_nav( $nav_id ) {
		global $wp_query,  $anews_allowed_html_array;
		if ( false == get_theme_mod( 't_p_older_posts', false ) ) { $t_p_older_posts = wp_kses(__( '<span class="meta-nav">&larr;</span> Older posts', 'anews' ), $anews_allowed_html_array );  } else { $t_p_older_posts = get_theme_mod( 't_p_older_posts' ); $t_p_older_posts = wp_kses(( '<span class="meta-nav">&larr;</span> '.$t_p_older_posts), $anews_allowed_html_array ); }
		if ( false == get_theme_mod( 't_p_newer_posts', false ) ) { $t_p_newer_posts = wp_kses(__( 'Newer posts <span class="meta-nav">&rarr;</span>', 'anews' ), $anews_allowed_html_array );  } else { $t_p_newer_posts = get_theme_mod( 't_p_newer_posts' ); $t_p_newer_posts = wp_kses(( $t_p_newer_posts.' <span class="meta-nav">&rarr;</span>'), $anews_allowed_html_array ); }
 ...
 */
function anews_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $site_description && ( is_home() || is_front_page() ) )
		...
		if ( false == get_theme_mod( 't_p_page', false ) ) { $t_p_page = esc_html__("Page %s", "anews");  } else { $t_p_page = get_theme_mod( 't_p_page' ); $t_p_page = $t_p_page.' %s'; }
...
add_filter( 'wp_title', 'anews_wp_title', 10, 2 );
 ...
 *
 */
function anews_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'anews_page_menu_args' );
