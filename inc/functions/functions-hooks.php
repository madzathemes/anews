...
function anews_css() {
	...
	wp_enqueue_style('anews-style', get_stylesheet_uri());
	...
	// Footer Colors
	 ...
	 if ( $custom_styles != '' ) {
	  $css = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $custom_styles);
		wp_add_inline_style( 'anews-style', $css );
	}

}
add_action( 'wp_enqueue_scripts', 'anews_css');

function anews_header_script() {

		$option = get_option("anews_theme_options");

		wp_enqueue_script( 'anews_script', get_template_directory_uri(). '/inc/js/scripts.js', array( 'jquery'), '', true );
		wp_localize_script( 'anews_script', 'ajax_posts', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'noposts' => esc_html__('No older posts found', 'anews'), ));
	    ...
	    function anews_fonts_url() {
	      ...
	      $theme_font = "Lato:400,900,700";
	         ...
	         */
	        if ( 'off' !== _x( 'on', 'Google font: on or off', 'anews' ) ) {
	            $font_url = add_query_arg( 'family', urlencode( ''. esc_attr($theme_font) .'' ), "//fonts.googleapis.com/css" );
	        }
	        return $font_url;
	    }
		}
    wp_enqueue_style( 'anews-fonts', anews_fonts_url(), array(), '1.0.0' );


}
add_action('wp_enqueue_scripts', 'anews_header_script');

function anews_admin_script() {
	wp_enqueue_style('anews-admin', get_template_directory_uri().'/inc/css/admin.css');
}
add_action('admin_enqueue_scripts', 'anews_admin_script');


add_filter('body_class','anews_class');
function anews_class($classes) {
